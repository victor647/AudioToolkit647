                ...
                if (System.IO.File.Exists(infoPath))
                {
                    ...
                    if (soundBankInfo != null)
                    {
                        try
                        {
                            ...
                            fileDescriptorList.AddRange(soundBankInfo.SoundBanks.SoundBank.Cast<AK.Wwise.InfoFile.FileDescriptorType>().Where(x => !string.IsNullOrEmpty(x.Path)));
                            fileDescriptorList.AddRange(soundBankInfo.StreamedFiles.File.Cast<AK.Wwise.InfoFile.FileDescriptorType>().Where(x => !string.IsNullOrEmpty(x.Path)));

                            foreach(var bank in soundBankInfo.SoundBanks.SoundBank)
                            {
                                fileDescriptorList.AddRange(bank.ReferencedStreamedFiles.File.Cast<AK.Wwise.InfoFile.FileDescriptorType>().Where(x => !string.IsNullOrEmpty(x.Path)));

                                foreach(var eventEntry in bank.IncludedEvents.Event)
                                {
                                    fileDescriptorList.AddRange(eventEntry.ReferencedStreamedFiles.File.Cast<AK.Wwise.InfoFile.FileDescriptorType>().Where(x => !string.IsNullOrEmpty(x.Path)));
                                }
                            }

                            var fileDescriptors = fileDescriptorList
                                .Distinct()
                                .Union(soundBankInfo.MediaFilesNotInAnyBank.File.Cast<AK.Wwise.InfoFile.FileDescriptorType>().Where(x => !string.IsNullOrEmpty(x.Path)));
