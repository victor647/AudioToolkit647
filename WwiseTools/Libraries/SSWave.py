...
class SWaveObject(object):
    ...
    ###### 转换采样率 ######
    # pydub方法
    def convert_framerate_pydub(self, result_path, convert_framerate, convert_bit):
        # 转换为44100，16bit
        input_1 = AudioSegment.from_wav(self.wav_path)

        out_put = input_1.set_frame_rate(convert_framerate)
        samplerate = int(convert_bit / 8)
        out_put = out_put.set_sample_width(samplerate)
        out_put.export(result_path, format='wav')
        print('Converted!')
    ...
    ###### 写Mark点 ######
    def writeMark(self, markers=None, loops=None, pitch=None, normalized=False):
        """
        Write a numpy array as a WAV file
        ...
        if loops or pitch:
            ...
            fid.write(
                struct.pack('<iiiiiIiiii', size, 0, 0, sampleperiod, midiunitynote, midipitchfraction, 0, 0, len(loops),
                            0))
            for i, loop in enumerate(loops):
                fid.write(struct.pack('<iiiiii', 0, 0, loop[0], loop[1], 0, 0))
    ...
    def voiceRecognition(self):
        '''
        暂不支持超过15s的音频
        :return:eg：[["君子", "00:00:00.270", "00:00:00.730"], ["国", "00:00:00.740", "00:00:00.990"]]

        '''

        url = 'http://172.29.58.44:5000/api/stt/query/search'
        files = {'file': open(self.wav_path, 'rb')}
    ...
    ## 根据AI转换的结果写Mark点 ##
    def voiceRecognitionMarker(self, voice_recognition_list):
        '''
        Windows的编码是GBK
        :param voice_recognition_list:
        :return:
        '''
        mark_list = []
        for i in voice_recognition_list:
            each_mark = {}
            sec = self.timeToSec(i[1])
            frame = int(sec * self.framerate)
            # label_ascii = bytes(i[0], 'UTF-8')
            label = i[0].encode('gbk')
            each_mark.update({'position': frame})
            each_mark.update({'label': label})
            mark_list.append(each_mark)

        self.writeMark(mark_list)
        print('Marked!')
